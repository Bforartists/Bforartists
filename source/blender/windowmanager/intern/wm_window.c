...
void wm_window_add_ghostwindows(wmWindowManager *wm)
{
	...
	for (win = wm->windows.first; win; win = win->next) {
		if (win->ghostwin == NULL) {
			...
			wm_window_add_ghostwindow(wm, "Bforartists", win);
		}
...
void WM_window_open_temp(bContext *C, rcti *position, int type)
{
	...
	if (sa->spacetype == SPACE_IMAGE)
		GHOST_SetTitle(win->ghostwin, IFACE_("Bforartists Render"));
	else if (ELEM(sa->spacetype, SPACE_OUTLINER, SPACE_USERPREF))
		GHOST_SetTitle(win->ghostwin, IFACE_("Bforartists User Preferences"));
	else if (sa->spacetype == SPACE_FILE)
		GHOST_SetTitle(win->ghostwin, IFACE_("Bforartists File View"));
	else
		GHOST_SetTitle(win->ghostwin, "Bforartists");
}
