                                     ...
                                     const bool do_append)
{
  ...
  if (!BKE_idtype_idcode_is_linkable(idcode) ||
    ...
    if (reports) {
      if (do_append) {
        BKE_reportf(reports, RPT_ERROR_INVALID_INPUT, "Can't append data '%s' of type '%s'", name, group);
      }
      else {
        BKE_reportf( reports, RPT_ERROR_INVALID_INPUT, "Can't link data '%s' of type '%s'", name, group);
      }
    }
    return false;
  }
