...
#include "BLI_math_constants.hh" /*BFA*/
...
const EnumPropertyItem buttons_context_items[] = {
    ...
    {BCONTEXT_STRIP, "STRIP", ICON_SEQUENCE, "Strip", "Strip Properties"}, /*BFA - icon*/
...
static void rna_def_space_outliner(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_filter_pose_bones_per_bone", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_negative_sdna(prop, nullptr, "filter", SO_FILTER_NO_POSE_BONE);
  RNA_def_property_ui_text(
      prop,
      "Pose Bones (Per Bone)",
      "Hide individual pose bones in the Outliner.\n"
      "Each bone is controlled by its own \"Hide in Outliner\" toggle in the Bone properties");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_OUTLINER, nullptr);
  ...
  RNA_def_property_ui_text(
      prop,
      "Pose Bones",
      "Show or hide the entire Pose Bones group of armatures in the Outliner");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_OUTLINER, nullptr);
}
...
static void rna_def_space_view3d_overlay(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna,
                          "gpencil_grid_offset",
                          PROP_FLOAT,
                          PROP_XYZ); /* BFA - keep has PROP_XYZ from PROP_DISTANCE */
...
static void rna_def_fileselect_asset_params(BlenderRNA *brna)
{
  ...
  RNA_def_property_boolean_funcs(
      prop,
      nullptr,
      "rna_FileAsset_instance_collection_set");  // bfa make append also update link button
...
static void rna_def_space_node_overlay(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "show_world_center", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, nullptr, "overlay.flag", SN_OVERLAY_SHOW_WORLD_CENTER);
  RNA_def_property_boolean_default(prop, true);
  RNA_def_property_ui_text(
      prop, "World Center", "Show canvas center X and Y axes in the node editor background");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE, nullptr);

  /*BFA - World Center overlay*/
  prop = RNA_def_property(srna, "world_center_alpha", PROP_FLOAT, PROP_FACTOR);
  RNA_def_property_float_sdna(prop, nullptr, "overlay.world_center_alpha");
  RNA_def_property_range(prop, 0.0f, 1.0f);
  RNA_def_property_float_default(prop, 0.25f);
  RNA_def_property_ui_text(
      prop, "World Center Alpha", "Transparency level for canvas center lines");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE, nullptr);
}
...
#  endif
  ...
  RNA_def_property_ui_text(prop, "Show", "Display node minimap navigation widget. Press to drag the view, scroll to zoom in and out, and click to snap view to that location.");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE_VIEW, nullptr);
  ...
  RNA_def_property_ui_text(prop, "Nodes in Frame", "Display minimap nodes parented to frames");
  RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE_VIEW, nullptr);
