...
static void rna_def_space_image_uv(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem sticky_mode_items[] = {
		{SI_STICKY_DISABLE, "DISABLED", ICON_STICKY_UVS_DISABLE, "Disabled", "Disabled\nSticky vertex selection disabled"},
	...
	RNA_def_property_ui_text(prop, "Display Modified Edges", "Display Modified Edges\nDisplay edges after modifiers are applied");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_IMAGE, NULL);
	...
	RNA_def_property_ui_text(prop, "Display Texture Paint UVs", "Display Texture Paint UVs\nDisplay overlay of texture paint uv layer");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_IMAGE, NULL);
...
static void rna_def_space_outliner(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Show Object Contents", "Show Object Contents\nShow what is inside the objects elements");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_OUTLINER, NULL);
	...
	RNA_def_property_ui_text(prop, "Show Other Objects", "Show Other Objects\nShow curves, lattices, light probes, fonts, ...");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_OUTLINER, NULL);
...
static void rna_def_space_view3d_shading(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Viewport Shading", "Viewport Shading\nMethod to display/shade objects in the 3D View");
	...
	RNA_def_property_ui_text(prop, "Studiolight Rotation", "Studiolight Rotation\nRotation of the studiolight around the Z-Axis");
...
static void rna_def_space_view3d_overlay(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Grid Lines", "Grid Lines\nNumber of grid lines to display in perspective view");
	...
	RNA_def_property_ui_text(prop, "Grid Scale Unit", "Grid Scale Unit\nGrid cell size scaled by scene unit system settings");
	...
	RNA_def_property_ui_text(prop, "Show Bones", "Show Bones\nDisplay bones (disable to show motion paths only)");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_float_default(prop, 0.5f);
	RNA_def_property_ui_text(prop, "Wireframe Threshold", "Wireframe Threshold\nAdjust the number of wires displayed (1 for all wires)");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_clear_flag(prop, PROP_ANIMATABLE);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_ui_text(prop, "Show Weight Contours", "Show Weight Contours\nShow contour lines formed by points with the same interpolated weight");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_ui_text(prop, "Draw Face Center", "Draw Face Center\nDisplay face center "
	...
	RNA_def_property_ui_text(prop, "Draw Sharp", "Draw Sharp\nDisplay sharp edges, used with the Edge Split modifier");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);

	prop = RNA_def_property(srna, "show_freestyle_edge_marks", PROP_BOOLEAN, PROP_NONE);
	RNA_def_property_boolean_sdna(prop, NULL, "overlay.edit_flag", V3D_OVERLAY_EDIT_FREESTYLE_EDGE);
	RNA_def_property_ui_text(prop, "Draw Freestyle Edge Marks", "Draw Freestyle Edge Marks\nDisplay Freestyle edge marks, used with the Freestyle renderer");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_ui_text(prop, "Draw Freestyle Face Marks", "Draw Freestyle Face Marks\nDisplay Freestyle face marks, used with the Freestyle renderer");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_float_default(prop, 1.0f);
	RNA_def_property_ui_text(prop, "Vertex Paint Opacity", "Vertex Paint Opacity\nOpacity of the vertex paint mode overlay");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_float_default(prop, 1.0f);
	RNA_def_property_ui_text(prop, "Weight Paint Opacity", "Weight Paint Opacity\nOpacity of the weight paint mode overlay");
	RNA_def_property_range(prop, 0.0f, 1.0f);
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
...
static void rna_def_space_view3d(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Lock Camera to View", "Lock Camera to View\nEnable view navigation within the camera view");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_ui_text(prop, "Show 3D Marker Names", "Show 3D Marker Names\nShow names for reconstructed tracks objects");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_VIEW3D, NULL);
	...
	RNA_def_property_ui_text(prop, "Overlay Settings", "Overlay Settings\nSettings for display of overlays in the 3D viewport");
...
static void rna_def_space_image(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Display Repeated", "Display Repeated\nDisplay the image repeated outside of the main view");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_IMAGE, NULL);
...
static void rna_def_space_sequencer(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Center-Cut Safe Areas", "Center-Cut Safe Areas\nShow safe areas to fit content in a different aspect ratio");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_SEQUENCER, NULL);
...
static void rna_def_space_text(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Visible Lines", "Visible Lines\nAmount of lines that can be visible in current editor");
	...
	RNA_def_property_ui_text(prop, "Replace Text", "Replace TextnText to replace selected text with using the replace tool");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_TEXT, NULL);
...
static void rna_def_space_graph(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Cursor X-Value", "Cursor X-Value\nGraph Editor 2D-Value cursor - X-Value component");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_GRAPH, NULL);

	prop = RNA_def_property(srna, "cursor_position_y", PROP_FLOAT, PROP_NONE);
	RNA_def_property_float_sdna(prop, NULL, "cursorVal");
	RNA_def_property_ui_text(prop, "Cursor Y-Value", "Cursor Y-Value\nGraph Editor 2D-Value cursor - Y-Value component");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_GRAPH, NULL);
...
static void rna_def_fileselect_params(BlenderRNA *brna)
{
	...
	static const EnumPropertyItem file_display_items[] = {
		{FILE_SHORTDISPLAY, "LIST_SHORT", ICON_SHORTDISPLAY, "Short List", "Short List\nDisplay files as short list"},
		{FILE_LONGDISPLAY,  "LIST_LONG", ICON_LONGDISPLAY, "Long List", "Long List\nDisplay files as a detailed list"},
	...
	static const EnumPropertyItem file_filter_idtypes_items[] = {
		...
		{FILTER_ID_AR, "ARMATURE", ICON_ARMATURE_DATA, "Armatures", "Armatures\nShow/hide Armature data-blocks"},
		...
		{FILTER_ID_GD, "GREASE_PENCIL", ICON_GREASEPENCIL, "Grease Pencil", "Grease Pencil\nShow/hide Grease pencil data-blocks"},
		...
		{FILTER_ID_PC, "PAINT_CURVE", ICON_CURVE_BEZCURVE, "Paint Curves", "Paint Curves\nShow/hide Paint Curve data-blocks"},
		{FILTER_ID_LP, "LIGHT_PROBE", ICON_LIGHTPROBE_CUBEMAP, "Light Probes", "Light Probes\nShow/hide Light Probe data-blocks"},
	...
	RNA_def_property_ui_text(prop, "Library Browser", "Library Browser\nWhether we may browse blender files' content or not");
	RNA_def_property_clear_flag(prop, PROP_EDITABLE);
	...
	RNA_def_property_ui_text(prop, "Filter BlenderBackup files", "Filter BlenderBackup files\nShow .blend1, .blend2, etc. files");
	...
	RNA_def_property_ui_text(prop, "Filter Blender IDs", "Filter Blender IDs\nShow .blend files items (objects, materials, etc.)");
	...
	RNA_def_property_flag(prop, PROP_ENUM_FLAG);
	RNA_def_property_ui_text(prop, "Filter ID types", "Filter ID types\nWhich ID types to show/hide, when browsing a library");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_FILE_PARAMS, NULL);
	...
	RNA_def_property_flag(prop, PROP_ENUM_FLAG);
	RNA_def_property_ui_text(prop, "Filter ID categories", "Filter ID categories\nWhich ID categories to show/hide, when browsing a library");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_FILE_PARAMS, NULL);
...
static void rna_def_space_filebrowser(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Filebrowser Parameter", "Filebrowser Parameter\nParameters and Settings for the Filebrowser");
...
static void rna_def_space_node(BlenderRNA *brna)
{
	...
	RNA_def_property_ui_text(prop, "Auto Render", "Auto Render\nRe-render and composite changed layers on 3D edits");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE_VIEW, NULL);
	...
	RNA_def_property_ui_text(prop, "Auto-offset Direction", "Auto-offset Direction\nDirection to offset nodes on insertion");
	RNA_def_property_update(prop, NC_SPACE | ND_SPACE_NODE_VIEW, NULL);
