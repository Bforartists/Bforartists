...
static void panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  if (wrap_method == MOD_SHRINKWRAP_PROJECT) {
    ...
    uiItemR(row, ptr, "use_negative_direction", UI_ITEM_NONE, nullptr, ICON_NONE);
    uiItemDecoratorR(row, ptr, "use_negative_direction", 0); /*bfa - decorator*/

    row = uiLayoutRow(col, true);
    uiLayoutSetPropSep(row, false); /* bfa - use_property_split = False */
    uiItemR(row, ptr, "use_positive_direction", UI_ITEM_NONE, nullptr, ICON_NONE);
    ...
    uiLayoutSetPropSep(row, false); /* bfa - use_property_split = False */
    uiItemS(row);
    uiItemR(row, ptr, "use_invert_cull", UI_ITEM_NONE, nullptr, ICON_NONE);
