...
static void panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  uiItemS(row);
  uiItemR(row, ptr, "use_object_screw_offset", 0, nullptr, ICON_NONE);
...
static void normals_panel_draw(const bContext * /*C*/, Panel *panel)
{
  ...
  uiLayoutSetPropSep(row, false); /* bfa - use_property_split = False */
  uiItemR(row, ptr, "use_normal_calculate", 0, nullptr, ICON_NONE);
  uiItemDecoratorR(row, ptr, "use_normal_calculate", 0); /*bfa - decorator*/
  row = uiLayoutRow(col, true);

  uiLayoutSetPropSep(row, false); /* bfa - use_property_split = False */
  uiItemR(row, ptr, "use_normal_flip", 0, nullptr, ICON_NONE);
