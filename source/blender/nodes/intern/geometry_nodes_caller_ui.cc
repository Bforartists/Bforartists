...
{
  std::string rna_path = "value";
  std::string identifier = socket.identifier;
  ...
  prop_row = &layout.row(true); /* BFA - removed the if else, we always want this to be true to float left*/
  ...
  if (show_attribute_input) {
    ...
    ctx.draw_attribute_toggle_fn(
        *prop_row, ICON_SPREADSHEET, socket); /*BFA - move toggle button besides property */
    prop_row->decorator(socket_props_ptr, std::optional<StringRefNull>(StringRefNull("value")), -1); /* BFA - draw inside prop_row */
  }
                                     ...
                                     const std::optional<StringRef> parent_name = std::nullopt)
{
  ...
  switch (type) {
    ...
    case SOCK_MASK: {
      row.prop_search(socket_props_ptr, "value", ctx.bmain_ptr, "masks", name, ICON_NONE); //BFA - WIP, missing icon
      break;
    }
    case SOCK_SOUND: {
      PropertyRNA *prop = RNA_struct_find_property(socket_props_ptr, "value");
