...
#include "BLI_math_color.hh"   // bfa - color row collection uchar to float4
#include "BLI_math_color_c.hh" // bfa - rgba_uchar_to_float
                                       ...
                                       TreeElement **te_edit)
{
  ...
  if (*starty + 2 * UI_UNIT_Y >= region->v2d.cur.ymin && *starty <= region->v2d.cur.ymax) {
    ...
    if (tselem->type == TSE_VIEW_COLLECTION_BASE) {
      ...
      /* Icons a bit higher. */
      if (TSELEM_OPEN(tselem, space_outliner)) {
        ui::icon_draw_alpha(float(icon_x) + 2 * ufac,
                            float(*starty) + 1 * ufac,
                                     ...
                                     int *io_start_y)
{
  ...
  const bool outliner_colored_collection_rows = (U.outliner_editor_flag &
                                                 USER_OUTLINER_COL_COLLECTION_ROWS) != 0;
  ...
  tree_iterator::all_open(*space_outliner, [&](const TreeElement *te) {
    ...
    if (outliner_colored_collection_rows) /*bfa - outliner colored collection rows*/
      if (outliner_is_collection_tree_element(te)) {
        ...
        rgba_uchar_to_float(col_collection,
                            (collection && collection->color_tag != COLLECTION_COLOR_NONE) ?
                                btheme->collection_color[collection->color_tag].color :
                                btheme->space_outliner.back);

        int depth = calculate_hierarchy_depth(
            te);                           // Calculate the hierarchy depth of the tree item
        ...
        ui::draw_roundbox_4fv(&rect, true, radius, background_color);
        ...
        if (collection && TSELEM_OPEN(tselem, space_outliner)) {
          ...
          /*RECTANGLES FOR CHILDREN*/
            ...
            rect.ymin = child_start_y - total_height + UI_UNIT_Y;
            rect.ymax = child_start_y + UI_UNIT_Y;
            ui::draw_roundbox_4fv(&rect, true, radius, nested_color);
          }
        }
