        ...
        int but_height)
{
	...
	if (newop) {
		...
		but = uiDefIconTextButO(
		        block, but_type, newop, WM_OP_INVOKE_DEFAULT, (id && !use_tab_but) ? ICON_ADD : ICON_ADD,
		...
		but = uiDefIconTextBut(
		        block, but_type, 0, (id && !use_tab_but) ? ICON_ADD : ICON_ADD,
        ...
        const char *newop, const char *unlinkop)
{
	...
	template_search_add_button_operator(block, newop, WM_OP_INVOKE_DEFAULT, ICON_ADD, editable); /*bfa - changed icon from icon duplicate to icon add*/
