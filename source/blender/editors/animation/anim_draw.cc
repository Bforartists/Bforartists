...
/* bfa 3d sequencer interactive scene strip gizmos overlay.
 *
 * Draws the scene-strip bar (with green/red retime handles, move/slip zones)
 * and the master-timeline scrub highlight at the bottom of the dope-sheet.
 * The interactive hit-areas are handled by the ACTION_GGT_scene_strip_gizmos
 * gizmo group; this draw call renders the visual only.
 *
 * Must be called with the region in `view2d_view_orthoSpecial` space (x in
 * frames, y in region pixels from the bottom), like the marker margin.
 */
void ANIM_draw_scene_strip_gizmos(const bContext *C, View2D *v2d)
{
  ...
  const Strip *master_strip = ed::vse::get_scene_strip_for_time_sync(sequencer_scene);
  if (!master_strip || !master_strip->scene) {
    return;
  }
  const Scene *active_scene = CTX_data_scene(C);
  if (master_strip->scene != active_scene) {
    return;
  }
  ARegion *region = CTX_wm_region(C);
  if (!region || region->winy < 112) {
    return;
  }

  const float ui_scale = UI_SCALE_FAC;
  const bool has_markers = !BLI_listbase_is_empty(&active_scene->markers);
  const float baseline = has_markers ? UI_MARKER_MARGIN_Y : 14.0f * ui_scale;
  ...
  immRectf(pos, float(sequencer_scene->r.cfra), baseline, float(sequencer_scene->r.cfra) + 1, baseline + timeline_height);
  ...
  if (ed) {
    for (const Strip &strip : ed->seqbase) {
      if (&strip == master_strip || strip.type != STRIP_TYPE_SCENE || strip.scene != active_scene) {
        continue;
      }
      const float left_handle = strip.left_handle();
      const float right_handle = strip.right_handle(sequencer_scene);
      float frame_in = seq::give_frame_index(sequencer_scene, &strip, left_handle) +
                       strip.scene->r.sfra + strip.anim_startofs;
      float frame_out = seq::give_frame_index(sequencer_scene, &strip, right_handle - 1) +
                        strip.scene->r.sfra + strip.anim_startofs;
      ...
      immRectf(pos, frame_in, y, frame_out, y + strip_height);
      immUniformColor4f(0.3f, 0.3f, 0.3f, 0.7f);
      immRectf(pos, frame_in, y, frame_in + 2.0f * ui_scale, y + strip_height);
      immRectf(pos, frame_out - 2.0f * ui_scale, y, frame_out, y + strip_height);
    }
  }

  /* Master strip (active). */
  const float left_handle = master_strip->left_handle();
  const float right_handle = master_strip->right_handle(sequencer_scene);
  float frame_in = seq::give_frame_index(sequencer_scene, master_strip, left_handle) +
                   master_strip->scene->r.sfra + master_strip->anim_startofs;
  float frame_out = seq::give_frame_index(sequencer_scene, master_strip, right_handle - 1) +
                    master_strip->scene->r.sfra + master_strip->anim_startofs;
  if (frame_in > frame_out) {
    std::swap(frame_in, frame_out);
  }

  const float y_strip = baseline + 14.0f * ui_scale;
  const float handle_width = 4.0f * ui_scale;
  const char highlight = space_action->runtime.scene_strip_gizmo_highlight;

  /* Strip base. */
  immUniformColor4f(0.1f, 0.1f, 0.1f, 0.8f);
  immRectf(pos, frame_in, y_strip, frame_out, y_strip + strip_height);

  /* Move / slip zones (visual feedback while hovering). */
  if (highlight == 2) {
    immUniformColor4f(0.35f, 0.55f, 0.75f, 0.55f);
    immRectf(pos, frame_in, y_strip + 6.0f * ui_scale, frame_out, y_strip + strip_height);
  }
  else if (highlight == 3) {
    immUniformColor4f(0.35f, 0.55f, 0.75f, 0.55f);
    immRectf(pos, frame_in, y_strip, frame_out, y_strip + 6.0f * ui_scale);
  }
  ...
  immRectf(pos, frame_in - handle_width, y_strip, frame_in + handle_width, y_strip + strip_height);
  ...
  immRectf(pos, frame_out - handle_width, y_strip, frame_out + handle_width, y_strip + strip_height);
