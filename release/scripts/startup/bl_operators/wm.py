...
class WM_OT_tool_set_by_id(Operator):
    ...
    def execute(self, context):
        ...
        if fn(context, space_type, self.name, as_fallback=self.as_fallback):
            ...
            self.report({'WARNING'}, f"Tool {self.name!r:s} not found for space {space_type!r:s}. Wrong Mode!")# bfa - added a wrong mode hint
            return {'CANCELLED'}
...
class WM_MT_splash(Menu):
    ...
    def draw(self, context):
        ...
        layout.label(text = "Bforartists 2 1.0.0 Release Candidate is based on Blender 2.83 Alpha")
