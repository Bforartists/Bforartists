...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        if obj is not None:
            ...
            # The different object tpyes and the available mode buttons
            if obj.type == 'MESH':

                if mode == 'OBJECT':
                    row.operator("object.mode_set", text="", icon = 'OBJECTMODE__ACTIVE').mode = 'OBJECT'
                else: 
                    row.operator("object.mode_set", text="", icon = 'OBJECT_DATAMODE').mode = 'OBJECT'

                if mode == 'EDIT':
                    row.operator("object.mode_set", text="", icon = 'EDITMODE_ACTIVE').mode = 'EDIT'
                else:
                    row.operator("object.mode_set", text="", icon = 'EDITMODE_HLT').mode = 'EDIT'

                if mode == 'SCULPT':
                    row.operator("object.mode_set", text="", icon = 'SCULPTMODE_ACTIVE').mode = 'SCULPT'
                else:
                    row.operator("object.mode_set", text="", icon = 'SCULPTMODE_HLT').mode = 'SCULPT'

                if mode == 'VERTEX_PAINT':
                    row.operator("object.mode_set", text="", icon = 'VPAINT_ACTIVE').mode = 'VERTEX_PAINT'
                else:
                    row.operator("object.mode_set", text="", icon = 'VPAINT_HLT').mode = 'VERTEX_PAINT'

                if mode == 'WEIGHT_PAINT':
                    row.operator("object.mode_set", text="", icon = 'WPAINT_ACTIVE').mode = 'WEIGHT_PAINT'
                else:
                    row.operator("object.mode_set", text="", icon = 'WPAINT_HLT').mode = 'WEIGHT_PAINT'

                if mode == 'TEXTURE_PAINT':
                    row.operator("object.mode_set", text="", icon = 'TPAINT_ACTIVE').mode = 'TEXTURE_PAINT'
                else:
                    row.operator("object.mode_set", text="", icon = 'TPAINT_HLT').mode = 'TEXTURE_PAINT'

                # Special case particle system. It is no object type but a modifier.
                object = bpy.context.object

                if object.modifiers:
                    if 'PARTICLE_SYSTEM' in [m.type for m in object.modifiers]:
                            if mode == 'PARTICLE_EDIT':
                                row.operator("object.mode_set", text="", icon = 'PARTICLEMODE_ACTIVE').mode = 'PARTICLE_EDIT'
                            else:
                                row.operator("object.mode_set", text="", icon = 'PARTICLEMODE').mode = 'PARTICLE_EDIT'
                    else:
                        row.operator("data.not_available", text="", icon = 'NOT_AVAILABLE') 
            ...
            if obj.type == 'ARMATURE':
                ...
                if mode == 'POSE':
                    row.operator("object.mode_set", text="", icon = 'POSE_ACTIVE').mode = 'POSE'
                else:
