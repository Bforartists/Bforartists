...
def export_volume(blender_material, export_settings):
    ...
    if attenuation_color_socket.socket is not None and isinstance(attenuation_color_socket.socket, bpy.types.NodeSocket):
    ...
    if isinstance(thickness_socket.socket, bpy.types.NodeSocket) and not thickness_socket.socket.is_linked:
        ...
        export_settings['current_paths']["node_tree." + thickness_socket.socket.path_from_id() + ".default_value"] = path_
