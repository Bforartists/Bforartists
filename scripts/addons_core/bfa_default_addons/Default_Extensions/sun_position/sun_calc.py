...
def move_sun(context):
    ...
    if sun_props.usage_mode == "ENVIRONMENT":
        ...
        if sun.bind_to_sun != sun_props.bind_to_sun:
            ...
            sun.bind.az_start_sun = sun_props.env_azimuth
            if env_tex:
                sun.bind.az_start_env = env_tex.texture_mapping.rotation.z

        if env_tex and sun_props.bind_to_sun:
            az = sun_props.env_azimuth - sun.bind.az_start_sun + sun.bind.az_start_env
            env_tex.texture_mapping.rotation.z = az

        if sun_props.sun_object:
            obj = sun_props.sun_object
            obj.location = get_sun_vector(
                sun_props.env_azimuth, sun_props.env_elevation) * sun_props.sun_distance

            rotation_euler = Euler((sun_props.env_elevation - pi/2,
                                    0, -sun_props.env_azimuth))
