...
def write_metarig(obj: ArmatureObject, layers=False, func_name="create",
    ...
    if widgets:
        ...
        if widget_map:
            # TODO: Untested change ... please confirm it works
            code.append("try:")
            code.append("    from bl_ext.blender_org.rigify.utils.widgets import widget_generator\n\n")
            code.append("except:")
            code.append("    try:")
            code.append("        from bl_ext.user_default.rigify.utils.widgets import widget_generator\n\n")
            code.append("    except:")
            code.append("        print(\nUnable to import rigify widget from default blender_org or local repositories.\n)")
            code += widget_code
    else:
        widget_map = {}
