...
def find_linked(context, strips, selected_strips):
    """
    Takes a list of strips and returns a list of all the strips
    and effects that are linked in time
    Args:
        - strips: a list of strips
    Returns a list of all the linked strips, but not the strips passed to the function
    """
    start, end = get_frame_range(strips, selected_strips)
    strips_in_range = [s for s in strips if is_in_range(context, s, start, end)]
    effects = (s for s in strips_in_range if s.type in SequenceTypes.EFFECT)
    selected_effects = (s for s in strips if s.type in SequenceTypes.EFFECT)

    linked_strips = []

    # Filter down to effects that have at least one of seq as input and
    # Append input strips that aren't in the source list to linked_strips
    for e in effects:
        if not hasattr(e, "input_2"):
            continue
        for s in strips:
            if e.input_2 == s:
                linked_strips.append(e)
                if e.input_1 not in strips:
                    linked_strips.append(e.input_1)
            elif e.input_1 == s:
                linked_strips.append(e)
                if e.input_2 not in strips:
                    linked_strips.append(e.input_2)
...
def find_strips_mouse(context, frame, channel, select_linked=False):
    """
    Finds a list of strips to select based on the frame and channel the mouse cursor is at
    Args:
        - frame: the frame the mouse or cursor is on
        - channel: the channel the mouse is hovering
        - select_linked: find and append the strips linked in time if True
    ...
    if select_linked:
        linked_strips = [
            s
            for s in context.strips
            if s.frame_final_start == strips[0].frame_final_start
            and s.frame_final_end == strips[0].frame_final_end
        ]
        strips.extend(linked_strips)
    return strips


def get_frame_range(strips, get_from_start=False):
    """
    Returns a tuple with the minimum and maximum frames of the
    list of passed strips.
    Args:
        - strips, the strips to use
    ...
    start = (
        1
        if get_from_start
        else min(strips, key=attrgetter("frame_final_start")).frame_final_start
    )
    end = max(strips, key=attrgetter("frame_final_end")).frame_final_end
    return start, end


def get_channel_range(strips):
    """
    Returns a tuple with the minimum and maximum channels of the
    list of passed strips.
    """
    start = min(strips, key=attrgetter("channel")).channel
    end = max(strips, key=attrgetter("channel")).channel
    return start, end
...
def slice_selection(context, strips, range_block=0):
    """
    ...
    block_end = 0
    broken_selection = []
    sorted_strips = sorted(strips, key=attrgetter("frame_final_start"))
...
def ripple_move(context, strips, duration_frames, delete=False):
    """
    ...
    """
    channels = {s.channel for s in strips}
    first_strip = min(strips, key=lambda s: s.frame_final_start)
    to_ripple = [
        s
        for s in context.strips
    ...
    if delete:
        delete_strips(strips)
    else:
        to_ripple = set(to_ripple + strips)

    move_selection(context, to_ripple, duration_frames, 0)


def find_strips_in_range(frame_start, frame_end, strips, find_overlapping=True):
    """
    ...
    Args:
        ...
        - strips (optional): only work with these strips.
        If it doesn't receive any, the function works with all the strips in the current context
...
def delete_strips(to_delete):
    """
    Deletes the list of strips `to_delete`
    """
...
def move_selection(context, strips, frame_offset, channel_offset=0):
    """
    Offsets the selected `strips` horizontally and vertically and preserves
