...
class VIEW3D_HT_header(Header):
    ...
    def draw(self, context):
        ...
        # BFA - custom operator from the Power User Tools to toggle the viewport silhouette
        row = layout.row(align=True)
...
class VIEW3D_MT_view_navigation(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("view3d.view_roll_set", text="Reset Roll") # BFA - WIP  - needs icon
        layout.operator("view3d.view_flip")  # BFA - WIP - needs icon
...
class VIEW3D_MT_mesh_add(Menu):
    ...
    def draw(self, _context):
        ...
        layout.operator("mesh.primitive_quad_sphere_add", text="Quad Sphere", icon='MESH_QUADSPHERE') # BFA - WIP - add to topbar
...
class VIEW3D_MT_object_context_menu(Menu):
    ...
    def draw(self, context):
        ...
        if obj is None:
            ...
            layout.operator("object.transform_axis_target") # BFA - WIP
...
class VIEW3D_PT_shading_color(Panel):
    bl_space_type = "VIEW_3D"
    bl_region_type = "HEADER"
    bl_label = "Wireframe Color"  # BFA - Made explicit
    bl_parent_id = "VIEW3D_PT_shading"
    ...
    def draw(self, context):
        ...
        if shading.type == "SOLID":
            layout.row().label(text="Object Color") # BFA - Made explicit
...
class VIEW3D_PT_mesh_paint_automasking(Panel):
    ...
    def draw(self, context):
        ...
        row = col.row()
        row.separator()
        row.prop(settings, "use_automasking_topology")

        # face masks automasking
        row = col.row()
        row.separator()
        row.prop(settings, "use_automasking_face_sets")
        ...
        row = split.row()
        row.separator()
        row.prop(settings, "use_automasking_cavity_inverted", text="Cavity (Inverted)")  # BFA - Capitalize label
        ...
        row = split.row()
        row.separator()
        row.prop(settings, "use_automasking_view_normal", text="View Normal")
        ...
        if settings.use_automasking_view_normal:
            ...
            row.use_property_split = False
            row.separator(factor=3.5)
            row.prop(settings, "use_automasking_view_occlusion", text="Occlusion")
            subcol = col.column(align=True)
            if not settings.use_automasking_view_occlusion:
                subcol.use_property_split = True
                row = subcol.row()
                row.separator(factor=3.5)
        ...
        row = split.row()
        row.separator()
        row.prop(settings, "use_automasking_start_normal", text="Area Normal")
