    ...
    def draw(self, context):
        ...
        if display_mode in {'LIBRARIES', 'ORPHAN_DATA'}:
            row.prop(space, "use_filter_id_type", text="", icon=(
                'FILTER' if space.use_filter_id_type else 'FILTER_OFF'))
...
class OUTLINER_PT_options_filter(Panel):
    ...
    def draw(self, context):
        ...
        if space.use_filter_object:
            ...
            row = sub.row()
            row.label(icon='BLANK1')
            row.prop(space, "use_filter_object_content", text="Object Contents", icon="OBJECT_CONTENTS")
            row = sub.row()
            row.label(icon='BLANK1')
            row.prop(space, "use_filter_children", text="Object Children", icon="CHILD")
